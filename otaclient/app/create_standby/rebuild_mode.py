...
class RebuildMode(StandbySlotCreatorProtocol):
    ...
    def __init__(
        ...
        *,
        ...
        self._update_phase_tracker = update_phase_tracker
        # path configuration
        self.boot_dir = Path(update_meta.boot_dir)
        self.standby_slot_mp = Path(update_meta.standby_slot_mount_point)
        self.active_slot_mp = Path(update_meta.active_slot_mount_point)
    ...
    def _cal_and_prepare_delta(self):
        self._update_phase_tracker(wrapper.StatusProgressPhase.REGULAR)

        # TODO 2: old_reg is not used currently
        logger.info("generating delta...")

        regular_inf_fname = self._otameta.regular.file
        dir_inf_fname = self._otameta.directory.file
    ...
    def _process_dirs(self):
        self._update_phase_tracker(wrapper.StatusProgressPhase.DIRECTORY)
    ...
    def _process_persistents(self):
        """NOTE: code of this method is copied from legacy mode"""
        from ..copy_tree import CopyTree

        self._update_phase_tracker(wrapper.StatusProgressPhase.PERSISTENT)
        ...
        persist_inf_fname = self._otameta.persistent.file
    ...
    def _process_symlinks(self):
        symlink_inf_fname = self._otameta.symboliclink.file
    ...
    def _apply_reginf_set(self, _hash: str, _regs_set: RegularInfSet):
        ...
        _downloaded_copy = self._download_dir / _hash
        _local_copy_available = _local_copy.is_file()
        _first_copy_prepared = _local_copy_available

        for is_last, _entry in _regs_set.iter_entries():
            _cur_stat = RegInfProcessedStats()
            _start = time.thread_time_ns()

            # prepare first copy for the hash group
            if not _local_copy_available:
                # TODO: what to do if there is no file in the download dir?
                shutil.move(str(_downloaded_copy), self._recycle_folder)
                _local_copy_available = True
            # record the size of this entry(by query the local copy)
            _cur_stat.size = _local_copy.stat().st_size

            # special treatment on /boot folder
            _mount_point = (
                self.standby_slot_mp if not _entry._base == "/boot" else self.boot_dir
            )

            # case 1: normal file
            if _entry.nlink == 1:
                _cur_stat.op = RegProcessOperation.OP_COPY
                if is_last:  # move the tmp entry to the dst
                    _entry.move_from_src(_local_copy, dst_mount_point=_mount_point)
                else:  # copy from the tmp dir
                    _entry.copy_from_src(_local_copy, dst_mount_point=_mount_point)
            # case 2: hardlink file
                ...
                _cur_stat.op = RegProcessOperation.OP_LINK
                _identifier = (
                    _entry.sha256hash if _entry.inode is None else _entry.inode
                )

                _dst = _entry.make_relative_to_mount_point(_mount_point)
                (
                    _hardlink_tracker,
                    _first_copy_in_hardlink_group,
                ) = self._hardlink_register.get_tracker(_identifier, _dst, _entry.nlink)

                if _first_copy_in_hardlink_group:
                    _entry.copy_from_src(_local_copy, dst_mount_point=_mount_point)
                else:
            ...
            # finish up
            _cur_stat.elapsed_ns = time.thread_time_ns() - _start
            stats_list.append(_cur_stat)
        ...
        self._stats_collector.report(*stats_list)
    ...
    def calculate_delta(self):
        # TODO: return the download delta here
        try:
