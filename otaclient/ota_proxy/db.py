...
class _OTACacheDB:
    ...
    def lookup_entry(self, fd: ColumnDescriptor, _input: Any) -> Optional[CacheMeta]:
        ...
        with self._con as con:
            ...
            if row := cur.fetchone():
                ...
                cur = con.execute(
                    (
                        f"UPDATE {self.TABLE_NAME} SET {CacheMeta.last_access.name}=? "  # type: ignore
                        f"WHERE {CacheMeta.url.name}=?"  # type: ignore
                    ),
    ...
    def rotate_cache(self, bucket: int, num: int) -> Optional[List[str]]:
        ...
        bucket_fn, last_access_fn = (
            CacheMeta.bucket.name,  # type: ignore
            CacheMeta.last_access.name,  # type: ignore
        )
...
def _proxy_wrapper(attr_n: str) -> Callable:
    ...
    def _wrapped(self, *args, **kwargs):
        ...
        _db_conn: _OTACacheDB = self._thread_local.db
        fut: Future = self._executor.submit(
            partial(getattr(_db_conn, attr_n), *args, **kwargs)
        )
        return fut.result()
...
def _proxy_cls_factory(
    proxy_cls: Type, *, wrapper: Callable[[str], Callable], source_cls: Type
) -> Type:
    ...
    for attr_n, attr in source_cls.__dict__.items():
        if not attr_n.startswith("_") and callable(attr):
            ...
            setattr(proxy_cls, attr_n, wrapper(attr_n))

    return proxy_cls


@partial(_proxy_cls_factory, wrapper=_proxy_wrapper, source_cls=_OTACacheDB)
class _ProxyBase:
    ...
    def __init__(self, db_f: str):
        ...
        def _initializer():
            ...
            self._thread_local.db = _OTACacheDB(db_f, init=False)
...
OTACacheDB = cast(Type[_OTACacheDB], _ProxyBase)
